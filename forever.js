...
var child = new (forever.Monitor)(path.join('index.js'), {
  silent: false,
  uid: process.title,
  pidFile: 'server.pid',
  max: 10,
...
child.on('exit:code', function (code) {
  console.error(process.title + ': detected script exited with code ' +
    code + '.');
});

// child.on('watch:restart', function (info) {
//   console.error(process.title + ': restarted because ' +
//     info.file + ' changed.');
// });

child.on('restart', function () {
  console.error(process.title + ': restarted script for ' +
    child.times + ' time.');
});
